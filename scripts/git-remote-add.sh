...
git remote add "$STAGE" "git@gitlab.$DOMAIN:wenex/client-sdk.git"
if [ $? -ne 0 ]; then
  echo "Error: Failed to add remote repository for client-sdk."
else
  git push "$STAGE" main
  echo "Remote repository for client-sdk added successfully."
fi
